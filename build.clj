;; Copyright (c) 2024-2026 Christian Weilbach. All rights reserved.
;; Build script for katzen — JAR, deploy to Clojars, GitHub release.
...
(defn jar [_]
  (b/write-pom {:class-dir class-dir
                ...
                :basis basis
                :src-dirs ["src"]
                :scm {:url "https://github.com/replikativ/katzen"
                      :connection "scm:git:git://github.com/replikativ/katzen.git"
                      :developerConnection "scm:git:ssh://git@github.com/replikativ/katzen.git"
                      :tag (str "v" version)}
                :pom-data [[:description
                            "Applied category theory for Clojure — GATs, ACSets, schema migration, wiring-diagram composition, with verified migration via the ansatz Lean kernel and a typed numerical compile path via raster."]
                           [:url "https://github.com/replikativ/katzen"]
                           [:licenses
                            [:license
                             [:name "MIT License"]
